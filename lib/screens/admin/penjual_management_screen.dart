...
class PenjualManagementScreen extends StatefulWidget {
  ...
  @override
  State<PenjualManagementScreen> createState() => _PenjualManagementScreenState();
}

class _PenjualManagementScreenState extends State<PenjualManagementScreen> {
  final _svc = FirebaseService();
  final _fmt = NumberFormat.currency(locale: 'id_ID', symbol: 'Rp ', decimalDigits: 0);
  List<UserModel> _penjual = [];
  ...
  Future<void> _load() async {
    ...
    final penjual = await _svc.getAllPenjual();
    if (!mounted) return;
    setState(() {
      _penjual = penjual;
      _loading = false;
  ...
  void _showAddDialog() {
    ...
    final uidKartuCtrl = TextEditingController();
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: _DialogField(controller: uidKartuCtrl, label: 'UID Kartu (opsional)'),
                      ),
                      const SizedBox(width: 8),
                      scanningUid
                          ? SizedBox(
                          ...
                          : IconButton(
                              ...
                              tooltip: 'Scan Kartu',
                              onPressed: _machines.isEmpty ? null : startScan,
                            ),
                    ],
                  ),
                  if (scanningUid)
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  if (namaCtrl.text.isEmpty || usernameCtrl.text.isEmpty || passCtrl.text.isEmpty) {
                    setDialogState(() => error = 'Nama, username, dan password wajib diisi');
                    return;
                  }
                  try {
                    await _svc.registerPenjual(
                      nama: namaCtrl.text.trim(),
                      username: usernameCtrl.text.trim(),
                      password: passCtrl.text,
                      uidKartu: uidKartuCtrl.text.trim().isNotEmpty ? uidKartuCtrl.text.trim() : null,
                    );
  ...
  void _showEditDialog(UserModel p) {
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  try {
                    final updates = <String, dynamic>{'nama': namaCtrl.text.trim()};
                    final uid = uidKartuCtrl.text.trim();
  ...
  Future<void> _confirmDelete(UserModel p) async {
    final ok = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        backgroundColor: AppTheme.cardColor,
        title: const Text('Hapus Penjual'),
        content: Text('Hapus akun "${p.nama}" (@${p.username})?'),
        actions: [
    ...
    if (ok == true) {
      await _svc.deleteUser(p.authUid!);
      _load();
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Manajemen Penjual'),
        actions: [
      ...
      body: _loading
          ...
          : _penjual.isEmpty
              ? const Center(child: Text('Belum ada penjual terdaftar'))
              : ListView.builder(
                  padding: const EdgeInsets.all(16),
                  itemCount: _penjual.length,
                  itemBuilder: (ctx, i) {
                    final p = _penjual[i];
                    return Container(
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                Text(p.nama, style: const TextStyle(fontWeight: FontWeight.w600)),
                                Text('@${p.username ?? '-'}',
                                    style: const TextStyle(color: Colors.white54, fontSize: 12)),
                                if (p.uidKartu != null && p.uidKartu!.isNotEmpty)
                                  Text('Kartu: ${p.uidKartu}',
